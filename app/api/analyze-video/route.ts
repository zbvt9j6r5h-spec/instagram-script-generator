...
export async function POST(request: NextRequest) {
  ...
  // 画像分析と同じカウンターで制限（ベンチマーク分析は合計3回/月）
  ...
  try {
    ...
    const rawImages: { data: string; type: string }[] = body.images

    if (!rawImages || rawImages.length === 0) {
      return NextResponse.json({ error: 'フレームが取得できませんでした' }, { status: 400 })
    }

    const imageContents = rawImages.map((img) => ({
      type: 'image' as const,
      source: {
...
アップロードされた画像は動画から等間隔で抽出したフレームです。これらを時系列として分析し、以下の項目をJSON形式で返してください。
...
JSONのみを返してください。説明文は不要です。`,
    ...
    return NextResponse.json({ error: '動画の分析に失敗しました' }, { status: 500 })
  }
}
